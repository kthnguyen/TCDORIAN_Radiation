...
IRDIR = WORKPLACE + r"\CCMP_Wind"
FIGDIR = IRDIR + r"\Figures"
...
for i in range(0,1):
    ...
    t_IR_path = IRDIR + "\CCMP_Wind_Analysis_" + t_time_no_hour + "_V02.0_L3.0_RSS.nc"
    t_IRimg = xr.open_dataset(t_IR_path)
    
    #10deg distance from center of TC
    lat_bounds = np.float32([-10, 50])
    lon_bounds = np.float32([260,360])
    
    t_IRimg_lat = t_IRimg.coords['latitude'].values
    t_IRimg_lon = t_IRimg.coords['longitude'].values
    
#    for i in range(0,t_IRimg_lon.size):
#        if t_IRimg_lon[i] >180:
#            t_IRimg_lon[i] = t_IRimg_lon[i]-360
    ...
    lat_inds = t_IRimg_lat[min_lat_inds:min_lat_inds+240]
    lon_inds = t_IRimg_lon[min_lon_inds:min_lon_inds+400]
...
fig.savefig(FIGDIR + r"\wind_" + t_time +".png",dpi=1000)
plt.close()
#plt.show()
# =============================================================================
